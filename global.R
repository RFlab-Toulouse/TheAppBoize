...
diffexptest<-function(toto,test="Wtest"){ 
  ...
  for (i in 1:max(1,ncol(toto)) ){
    ...
    auc[i]<-auc(pROC::roc(group,toto[,i],quiet=TRUE))
  ...
  #model <- randomForest(learningmodel[,-1],learningmodel[,1],ntree=500,
   #                           importance=T,keep.forest=T)
    ...
    if (modelparameters$adjustval){
      ...
      auc<-auc(pROC::roc(as.vector(classval), as.vector(scoreval),quiet=T))
...
testmodel<-function(model,modeltype,tab,validation,criterionimportance,criterionmodel,fstype){
  ...
  if(modeltype=="svm"){
    ...
    if(criterionmodel=="auc"){
      if (fstype=='learn'){aucmod<-auc(pROC::roc(tab[,1], as.vector(model$decision.values),quiet=T))}
      if (fstype=='val'){
        print("")
    ...
    for(i in 1:length(lessimportantevar)){
      ...
      if(criterionmodel=="auc"){
        test[i]<-auc(pROC::roc(tabdiff2[,1], as.vector(resmodeldiff$decision.values),quiet=T))-aucmod}
    }}
  if(modeltype=="randomforest"){
    if(criterionmodel=="BER"){bermod<-BER(class = tab[,1],classpredict = model$predicted)}
    if(criterionmodel=="auc"){aucmod<-auc(pROC::roc(tab[,1], as.vector(model$votes[,1]),quiet=T))}
    for(i in 1:length(lessimportantevar)){
      tabdiff2<-tab[,-lessimportantevar[i]]
      tabdiff2<-as.data.frame(tabdiff2[,c(colnames(tabdiff2)[1],sort(colnames(tabdiff2[,-1])))])
      tabdiff2<-as.data.frame(tabdiff2[sort(rownames(tabdiff2)),])
      ...
      if(criterionmodel=="auc"){
        test[i]<-auc(pROC::roc(tabdiff2[,1], as.vector(resmodeldiff$votes[,1]),quiet=T))-aucmod}
    }
  }
...
testparametersfunction<-function(learning,validation,tabparameters){
  ...
  for (i in 1:nrow(tabparameters)){
    ...
    if(parameters$model!="nomodel"){
      ...
      #auclearning
      results[i,4]<-round(as.numeric(auc(pROC::roc(resmodel$datalearningmodel$reslearningmodel$classlearning,resmodel$datalearningmodel$reslearningmodel$scorelearning,quiet=T))),digits = 3)
      #sensibilitylearning
      ...
      #aucvalidation
      results[i,1]<-round(as.numeric(auc(pROC::roc(resmodel$datavalidationmodel$resvalidationmodel$classval,resmodel$datavalidationmodel$resvalidationmodel$scoreval,quiet=T))),digits = 3)
